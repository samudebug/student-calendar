...
export class TasksService {
  ...
  async createTask(classId: string, body: {name: string, deliverDate: Date, notes: string}) {
    await this.api.post(`/classes/${classId}/tasks`, body);
    this.getTasks(classId);
  }

  async updateTask(classId: string, taskId: string, body: Partial<{name: string, deliverDate: Date, notes: string}>) {
    await this.api.patch(`/classes/${classId}/tasks/${taskId}`, body);
    this.getTasks(classId);
  }

  async getTask(classId: string, taskId: string) {
    return this.api.get<Task & {student: Student}>(`/classes/${classId}/tasks/${taskId}`);
  }
